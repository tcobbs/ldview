...
void LDHtmlInventory::writeHeader(FILE *file)
{
	...
	if (writeExternalCss())
	{
		fprintf(file, "<link href=\"LDView.css\" title=\"Standard\" "
...
void LDHtmlInventory::writePartHeaderCell(FILE *file)
{
	...
	fprintf(file, "			<th colspan=\"%d\">Part</th>\n", partColumns);
}

void LDHtmlInventory::writeDescriptionHeaderCell(FILE *file)
{
...
void LDHtmlInventory::writeColorHeaderCell(FILE *file)
{
	fprintf(file, "			<th colspan=\"2\">Color</th>\n");
}

void LDHtmlInventory::writeQuantityHeaderCell(FILE *file)
{
...
void LDHtmlInventory::writeHeaderCell(FILE *file, LDPartListColumn column)
{
	...
	case LDPLCDescription:
		writeDescriptionHeaderCell(file);
		break;
	case LDPLCColor:
		writeColorHeaderCell(file);
		break;
	case LDPLCQuantity:
		writeQuantityHeaderCell(file);
		break;
	}
}
	...
	int colorNumber)
{
	...
	if (m_partImages)
	{
		...
		if (official)
		{
				fprintf(file, "<img %salt=\"View part on Peeron\" "
				"title=\"View part on Peeron\" "
				"src=\"http://media.peeron.com/ldraw/images/%d/100/%s.png\">",
				imgStyle.c_str(), peeronColorNumber, partName.c_str());
		}
		else
		{
			fprintf(file, "View part on Peeron");
		}
		fprintf(file, "</a></td>\n");
	}
	...
	const LDPartCount &partCount)
{
	if (m_columnMap[LDPLCDescription])
	{
		...
		{
			fprintf(file, "			<td>&lt;No description&gt;</td>\n");
		}
	}
}

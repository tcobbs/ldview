...
if [ -f /etc/fedora-release -o -f /etc/centos-release -o -f /etc/oracle-release ] ; then
	for pkg in qt5-qtbase-devel gcc gl2ps tinyxml zlib libpng-devel libjpeg-turbo-devel mesa-libOSMesa-devel hostname which ; do
	...
	for pkg in qtbase5-dev gcc libgl2ps-dev libtinyxml-dev zlib1g libpng-dev libjpeg-dev libosmesa6-dev hostname which ; do
	...
	for pkg in lib64qt5base5-devel gcc lib64gl2ps-devel lib64tinyxml-devel lib64zlib1 lib64png-devel lib64jpeg-devel lib64osmesa-devel hostname which ; do
...
#	pacman -Sy --noconfirm grep awk >/dev/null
	for pkg in qt5-base gcc gl2ps tinyxml zlib libpng libjpeg-turbo mesa hostname which ; do
	...
	for pkg in libqt5-qtbase-devel gcc gl2ps-devel tinyxml-devel zlib libpng16-compat-devel libjpeg8-devel Mesa-devel hostname which ; do
	...
	apk update -q
	for pkg in qt5-qtbase-dev gcc gl2ps tinyxml-dev zlib libpng libjpeg-turbo mesa-osmesa hostname which ; do
